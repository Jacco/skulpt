...
_ignorecase_fixes = None # JPK fix me {i: tuple(j for j in t if i != j)
...
def _compile(code, pattern, flags):
    ...
    if (flags & SRE_FLAG_IGNORECASE and
        ...
        fixes = None
    # JPK SubPattern not iterable
    looper = pattern
    if not isinstance(looper, list):
    	looper = looper.data
    for op, av in looper:
...
def _optimize_charset(charset, fixup, fixes, isunicode):
    ...
    # JPK charmap = bytes(charmap) # should be hashable
    charmap = list(charmap)
    comps = {}
    mapping = list() # JPK bytearray(256)
    for i in range(255):
    	mapping.append('\0')
    block = 0
    data = list() # JPK bytearray()
